    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 150, background: "#0a0e0c", fontFamily: "'SF Pro Display', -apple-system, BlinkMacSystemFont, sans-serif", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      >
        ...
        <div style={slideBase(2)}>
          ...
          {/* Google — inactive */}
          <button onClick={() => { setGoogleToast(true); setTimeout(() => setGoogleToast(false), 2500); }} style={{ width: "100%", padding: 15, borderRadius: 14, background: "rgba(255,255,255,0.5)", border: "none", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, cursor: "pointer", marginBottom: 8, opacity: 0.6 }}>
          ...
          {/* Google toast */}
          {googleToast && (
            <div style={{ textAlign: "center", marginBottom: 8, padding: "8px 14px", borderRadius: 10, background: "rgba(251,191,36,0.1)", border: "1px solid rgba(251,191,36,0.2)", animation: "feedbackSlideIn 0.2s ease-out" }}>
              <span style={{ fontSize: 12, color: "#FBBF24", fontWeight: 600 }}>Logowanie Google wkrótce dostępne</span>
            </div>
          )}
