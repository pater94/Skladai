...
export default function OnboardingLogin({ onSkip, startSlide = 0 }: OnboardingLoginProps) {
  ...
  const [loginBusy, setLoginBusy] = useState<"apple" | "google" | null>(null);
      ...
      >
        ...
        <div style={slideBase(0)} data-scrollable="true">
          ...
          <div style={{ textAlign: "center", marginBottom: 8, position: "relative" }}>
            ...
            <div style={{ fontSize: 13, fontWeight: 700, color: "rgba(var(--fg-rgb, 255,255,255),0.55)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 6 }}>
              Skład<span style={{ color: "var(--c-mint, #6efcb4)" }}>AI</span>
            </div>
            <div style={{ fontSize: 22, fontWeight: 900, letterSpacing: "-0.03em", color: "var(--fg, #fff)", lineHeight: "28px" }}>
              Twój osobisty{" "}
          ...
          <div style={{ marginTop: 14 }}>
            {cards.map((c, i) => (
              <div key={i} onClick={() => setActiveCard(i)} style={{
                ...
                background: activeCard === i ? c.bg : "rgba(var(--fg-rgb, 255,255,255),0.015)",
                border: activeCard === i ? `1.5px solid ${c.color}25` : "1.5px solid rgba(var(--fg-rgb, 255,255,255),0.04)",
                transform: activeCard === i ? "scale(1.01)" : "scale(1)",
                transition: "all 0.5s cubic-bezier(.4,0,.2,1)",
              }}>
                <div style={{ width: 40, height: 40, borderRadius: 12, background: activeCard === i ? `${c.color}15` : "rgba(var(--fg-rgb, 255,255,255),0.03)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0, transition: "all 0.4s ease" }}>{c.icon}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 800, color: activeCard === i ? "rgba(var(--fg-rgb, 255,255,255),0.95)" : "rgba(var(--fg-rgb, 255,255,255),0.55)", transition: "color 0.3s ease", marginBottom: 2 }}>{c.title}</div>
                  <div style={{ fontSize: 12.5, lineHeight: "17px", color: activeCard === i ? "rgba(var(--fg-rgb, 255,255,255),0.6)" : "rgba(var(--fg-rgb, 255,255,255),0.55)", transition: "color 0.3s ease" }}>{c.desc}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={slideBase(1)} data-scrollable="true">
          ...
          <div style={{ textAlign: "center", marginBottom: 18, position: "relative" }}>
            ...
            <div style={{ fontSize: 22, fontWeight: 900, color: "var(--fg, #fff)", letterSpacing: "-0.03em", marginBottom: 6 }}>
              Sprawdzone przez tysiące
            </div>
            <div style={{ fontSize: 13, color: "rgba(var(--fg-rgb, 255,255,255),0.6)" }}>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 20 }}>
            {[
              ...
              <div key={i} style={{ flex: 1, padding: "16px 8px", borderRadius: 14, textAlign: "center", background: "rgba(var(--fg-rgb, 255,255,255),0.03)", border: "1px solid rgba(var(--fg-rgb, 255,255,255),0.06)" }}>
                <div style={{ fontSize: 22, fontWeight: 900, color: s.color, marginBottom: 5, letterSpacing: "-0.02em" }}>{s.num}</div>
                <div style={{ fontSize: 10.5, color: "rgba(var(--fg-rgb, 255,255,255),0.7)", lineHeight: "14px" }}>{s.desc}</div>
              </div>
            ))}
          </div>

          <div style={{ padding: "16px 18px", borderRadius: 16, background: "rgba(var(--fg-rgb, 255,255,255),0.03)", border: "1px solid rgba(var(--fg-rgb, 255,255,255),0.06)", position: "relative", minHeight: 110 }}>
            ...
            {[
              ...
              <div key={i} style={{ position: i === 0 ? "relative" : "absolute", top: i === 0 ? undefined : 16, left: i === 0 ? undefined : 18, right: i === 0 ? undefined : 18, opacity: activeQuote === i ? 1 : 0, transition: "opacity 0.4s ease" }}>
                <div style={{ fontSize: 13.5, color: "rgba(var(--fg-rgb, 255,255,255),0.75)", lineHeight: "20px", fontStyle: "italic", padding: "4px 0" }}>
                  {q.text}
                </div>
                <div style={{ fontSize: 11.5, color: "rgba(var(--fg-rgb, 255,255,255),0.6)", marginTop: 8, fontStyle: "normal" }}>
                  {q.author}
                </div>
              </div>
            ))}
        ...
        <div style={slideBase(2)} data-scrollable="true">
          ...
          <div style={{ textAlign: "center", marginBottom: 16, position: "relative" }}>
            ...
            <div style={{ fontSize: 24, fontWeight: 900, color: "var(--fg, #fff)", letterSpacing: "-0.03em", lineHeight: "30px", marginBottom: 6 }}>
              Jedz mądrzej.{" "}
            ...
            <div style={{ fontSize: 14, color: "rgba(var(--fg-rgb, 255,255,255),0.6)" }}>
          ...
          <button
            ...
            style={{ width: "100%", padding: 15, borderRadius: 14, background: "rgba(var(--fg-rgb, 255,255,255),0.5)", border: "none", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, cursor: loginBusy ? "not-allowed" : "pointer", marginBottom: 8, opacity: loginBusy && loginBusy !== "google" ? 0.5 : 1 }}
          >
            ...
            <span style={{ fontWeight: 700, fontSize: 14.5, color: "#333" }}>
              {loginBusy === "google" ? "Łączenie..." : "Kontynuuj z Google"}
            </span>
          </button>
          ...
          <button
            ...
            disabled={loginBusy !== null}
            style={{ width: "100%", padding: 15, borderRadius: 14, background: "#000", border: "1px solid rgba(var(--fg-rgb, 255,255,255),0.12)", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, cursor: loginBusy ? "not-allowed" : "pointer", marginBottom: 12, opacity: loginBusy && loginBusy !== "apple" ? 0.5 : 1 }}
          >
            ...
            <span style={{ fontWeight: 700, fontSize: 14.5, color: "var(--fg, #fff)" }}>
              {loginBusy === "apple" ? "Łączenie..." : "Kontynuuj z Apple"}
            </span>
          </button>

          {/* Skip */}
          <div style={{ textAlign: "center" }}>
            <button onClick={onSkip} style={{ background: "none", border: "none", fontSize: 12.5, color: "rgba(var(--fg-rgb, 255,255,255),0.7)", cursor: "pointer" }}>Pomiń — korzystaj bez konta</button>
          </div>

          {/* Privacy */}
          <div style={{ textAlign: "center", marginTop: 10, fontSize: 10, color: "rgba(var(--fg-rgb, 255,255,255),0.6)" }}>
        ...
        <div style={{ display: "flex", gap: 6, justifyContent: "center", marginBottom: 16 }}>
          {[0, 1, 2].map(i => (
            <div key={i} onClick={() => setSlide(i)} style={{
              ...
              background: slide === i ? "var(--c-mint, #6efcb4)" : "rgba(var(--fg-rgb, 255,255,255),0.1)",
