...
export default function ProfileSetup({ onComplete, onSkip, existingProfile }: Props) {
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "var(--bg, #0a0e0c)" }}>
      {/* Header */}
      ...
      <div style={{ maxWidth: 448, margin: "0 auto", padding: "0 20px", marginTop: -40, paddingBottom: 40, position: "relative", zIndex: 20 }}>
        <div style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 24, padding: 24 }}>
          ...
          <div style={{ display: "flex", gap: 12, marginTop: 32 }}>
            ...
            {step < 3 ? (
              <button onClick={() => setStep(step + 1)}
                style={{ flex: 1, padding: 14, borderRadius: 14, fontSize: 14, fontWeight: 700, cursor: "pointer", background: "linear-gradient(135deg, var(--c-mint, #6efcb4), var(--c-green-2, #3dd990))", border: "none", color: "var(--c-ink, #0a0f0d)", boxShadow: "0 4px 20px rgba(var(--c-mint-rgb, 110,252,180),0.2)" }}>
                Dalej →
              </button>
            ) : (
